# Driver for legacy Windows systems

This page provides help how to use virtio-win, spice-guest-agent, and qemu-guest-agent on legacy windows systems
...
6. **Launch the Installation Wizard:**
   - Inside the Virtio-Win ISO, locate the `spice-guest-tools-0.141.exe` file and run it by double-clicking it. This file contains the Virtio-Win drivers, spice-agent, and the Installation Wizard.

7. **Driver Installation:**
   - Click "Install" to proceed. The Installation Wizard will begin to install the selected drivers. You will see progress bars for each driver component being installed.
   - Wait until installation completed to continue to the next steps.

8. **Install qemu-guest-agent:**
   - Inside the Virtio-Win ISO, locate the `guest-agent` directory who contains installer for qemu-guest-agent. double click `qemu-ga-(arch).msi` to install it. i386 for windows 32-bit, and x86_64 is for 64-bit windows.

9. **Reboot the VM:**
   - After the drivers and qemu-guest-agent are installed, then to reboot the VM for the changes to take effect. Don't forget to save your work.
...
## Windows XP - Installing virtio-blk drivers
...
2. **Attach the Virtio-Win Floppy to the VM:**
   - Ensure your VM is powered on and running.
   - In the VM management software (e.g., VirtualBox, VMware, QEMU, etc.), locate the option to attach the Virtio-Win Floppy as a virtual Floppy drive to the VM. The specific steps may vary depending on the virtualization software you're using.

3. **Boot and install driver**
   - Boot your Windows XP VM, Press `F6` to install the driver, then press `S` when prompted, and then choose `RedHat VirtIO BLOCK Disk Device`. Choose WinXP if you use XP and Win2003 if you use Server 2003.
   - Continue your Windows XP installation like usual. Press `F8` to agree the license, and press enter to disk formatting menu, choose `NTFS (Quick)` for faster formatting, and so on.
